...
# code from https://github.com/wolfhong/LunarCalendar/blob/master/lunarcalendar/converter.py
module Converter # Calendars.LunarCalendars
    using ..LunarCalendars: Lunar, Date, year, month, day
    ...
    function SolarFromInt(g)::Date
        y = fld(10000 * g + 14780, 3652425)
        ddd = g - (365 * y + fld(y, 4) - fld(y, 100) + fld(y, 400))
        if ddd < 0
            y -= 1
            ddd = g - (365 * y + fld(y, 4) - fld(y, 100) + fld(y, 400))
        end
        mi = fld(100 * ddd + 52, 3060)
        mm = (mi + 2) % 12 + 1
        y += fld(mi + 2, 12)
        dd = ddd - fld(mi * 306 + 5, 10) + 1
        return Date(y, mm, dd)
    end
    ...
    function SolarToInt(y, m, d)
        m = (m + 9) % 12
        y -= fld(m, 10)
        return 365 * y + fld(y, 4) - fld(y, 100) + fld(y, 400) + fld(m * 306 + 5, 10) + (d - 1)
    end

    function Lunar2Solar(lunar::Lunar)::Date
        days = Converter.lunar_month_days[lunar.year - Converter.lunar_month_days[1] + 1]
        ...
        for i in range(0, length=loopend)
            offset += GetBitInt(days, 1, 12 - i) == 1 ? 30 : 29
        end
        offset += lunar.day
        solar11 = Converter.solar_1_1[lunar.year - Converter.solar_1_1[1] + 1]
